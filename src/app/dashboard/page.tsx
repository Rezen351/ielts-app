...
export default function DashboardPage() {
  ...
  // Check Write States
  const [checkWritePrompt, setCheckWritePrompt] = useState<any>(null);
  const [checkWriteSentence, setCheckWriteSentence] = useState('');
  const [checkWriteLoading, setCheckWriteLoading] = useState(false);
  const [checkWriteEvaluation, setCheckWriteEvaluation] = useState<any>(null);
  ...
  useEffect(() => {
    ...
    if (savedUser) {
      ...
      fetchCheckWritePrompt();
    }
  }, []);

  const fetchCheckWritePrompt = async () => {
    try {
      const response = await fetch('/api/user/insights/check-write');
      const data = await response.json();
      if (data.success) {
        setCheckWritePrompt(data);
      }
    } catch (err) {
      console.error("Failed to fetch check write prompt");
    }
  };

  const handleCheckWriteSubmit = async () => {
    if (!checkWriteSentence.trim() || checkWriteSentence.split(' ').length < 3) {
      toast.error("Please write a bit more.");
      return;
    }

    setCheckWriteLoading(true);
    try {
      const response = await fetch('/api/user/insights/check-write', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          sentence: checkWriteSentence,
          prompt: checkWritePrompt.prompt
        })
      });
      const data = await response.json();
      if (data.success) {
        setCheckWriteEvaluation(data.evaluation);
        toast.success("Sentence checked!");
      }
    } catch (err) {
      toast.error("Failed to check sentence");
    } finally {
      setCheckWriteLoading(false);
    }
  };
...
<main className={`flex-1 flex flex-col min-w-0 order-1 md:order-none ${sidebarOpen ? 'md:ml-0 -ml-0 blur-sm pointer-events-none md:pointer-events-auto' : ''}`}>
        ...
        <div className="p-4 md:p-8 max-w-6xl mx-auto w-full space-y-6 md:space-y-8 pb-20">
          ...
          {(user?.settings?.showEnglishCheck !== false) && (
          ...
          {/* Check Write Section */}
          {(user?.settings?.showCheckWrite !== false) && (
            <section className="animate-in fade-in duration-1000 slide-in-from-bottom-4 w-full">
              <Card className="border-0 shadow-xl bg-white rounded-[32px] overflow-hidden">
                <CardContent className="p-6 md:p-10 space-y-8">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <Badge variant="secondary" className="bg-blue-50 text-blue-700 border-blue-100 font-black uppercase tracking-widest text-[10px] px-3 py-1 mb-1">
                        Check Write: {checkWritePrompt?.topic || 'Daily Practice'}
                      </Badge>
                      <h3 className="text-lg font-bold text-slate-800 leading-tight">
                        {checkWritePrompt?.prompt || 'Write something about your day.'}
                      </h3>
                    </div>
                  </div>
                  ...
                  <div className="space-y-4">
                    <textarea 
                      value={checkWriteSentence}
                      onChange={(e) => setCheckWriteSentence(e.target.value)}
                      placeholder="Write 1-2 sentences here..."
                      className="w-full h-24 rounded-[24px] border border-slate-100 bg-slate-50/50 p-6 text-sm md:text-base font-medium focus:ring-4 focus:ring-blue-100 focus:border-blue-400 outline-none transition-all resize-none shadow-inner"
                    />
                    <div className="flex flex-col sm:flex-row gap-3">
                      <Button 
                        onClick={handleCheckWriteSubmit}
                        disabled={checkWriteLoading || !checkWriteSentence.trim()}
                        className="bg-slate-900 hover:bg-slate-800 text-white rounded-2xl h-12 px-8 font-bold shadow-lg shadow-slate-200 transition-transform active:scale-95"
                      >
                        {checkWriteLoading ? <Loader2 className="w-4 h-4 animate-spin mr-2" /> : <Sparkles className="w-4 h-4 mr-2" />}
                        Check Sentence
                      </Button>
                      <Button 
                        variant="ghost"
                        onClick={() => {
                          setCheckWriteSentence('');
                          setCheckWriteEvaluation(null);
                          fetchCheckWritePrompt();
                        }}
                        className="rounded-2xl text-slate-400 font-bold text-[10px] uppercase tracking-widest"
                      >
                        New Prompt
                      </Button>
                    </div>
                  ...
                  {checkWriteEvaluation && (
                    <div className="p-6 md:p-8 rounded-[28px] bg-slate-50 border border-slate-100 space-y-6 animate-in zoom-in-95 duration-500">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          <div className={`w-3 h-3 rounded-full ${checkWriteEvaluation.isCorrect ? 'bg-green-500' : 'bg-amber-500'}`} />
                          <span className="text-[10px] font-black uppercase tracking-widest text-slate-500">
                            {checkWriteEvaluation.isCorrect ? 'Excellent Accuracy' : 'Feedback Received'}
                          </span>
                        </div>
                        <div className="flex items-center gap-1.5 bg-blue-600 text-white px-3 py-1 rounded-full">
                           <Trophy className="w-3.5 h-3.5" />
                           <span className="text-xs font-black">Band {checkWriteEvaluation.score}</span>
                        </div>
                      </div>

                      <div className="space-y-4">
                        <p className="text-sm md:text-base font-bold text-slate-800 leading-relaxed italic">
                          "{checkWriteEvaluation.feedback}"
                        </p>
                        
                        <div className="space-y-2 pt-4 border-t border-slate-200">
                          <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Better Version:</p>
                          <p className="text-sm md:text-base font-bold text-blue-700 leading-relaxed">
                            {checkWriteEvaluation.improvement}
                          </p>
                        </div>
                      </div>
                    </div>
